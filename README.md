<h1 align="center">Hi, I'm Nikunj Shah 👋</h1>
<h3 align="center">AI & Machine Learning Intern | Data Science Enthusiast | Full-Stack ML Developer</h3>

<p align="center">
  <a href="https://github.com/NIKUNJPS">
    <img src="https://github.com/NIKUNJPS/NIKUNJPS/blob/main/Profile%20Banner.png" />
  </a>
</p>

---

## 🎯 About Me

- 🎓 3rd Year **B.Tech IT Student**
- 🤖 Strong interest in **AI, Machine Learning & NLP**
- 🧠 Experience building **real-world ML projects**
- 🚀 Passionate about learning, experimenting, and deploying models
- 🎯 Actively seeking **AI / ML / Data Science Internship**

> *“Code with purpose. Build with impact.”*

---

## 🧠 Skills & Expertise

### 🤖 AI & Machine Learning
`Machine Learning` • `Deep Learning` • `NLP`  
`Scikit-Learn` • `TensorFlow` • `Model Evaluation`

### 📊 Data Analytics & Visualization
`Pandas` • `NumPy` • `Matplotlib` • `Power BI`

### 🌐 Backend & Full-Stack
`FastAPI` • `Django` • `REST APIs` • `React`

### ⚙️ DevOps & Tools
`Docker` • `Git` • `GitHub` • `VS Code`

---

## 🧰 Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,cpp,java,js,react,fastapi,django,tensorflow,docker,git,github,vscode&perline=6" />
</p>

---

## 📊 GitHub Analytics

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=NIKUNJPS&show_icons=true&theme=tokyonight&hide_border=true" height="165"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=NIKUNJPS&theme=tokyonight&hide_border=true" height="165"/>
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=NIKUNJPS&layout=compact&theme=tokyonight&hide_border=true" height="150"/>
</p>

---

## 🏆 Achievements

- 🥇 **1st Prize Winner** – Department Level Project Competition  
- 📌 Built **end-to-end ML systems** with FastAPI & Docker  
- 📈 Experience with **data-driven decision making**

---

## ✨ Inspiration

> *"I have no special talent. I am only passionately curious."*  
> — **Albert Einstein**

---

## 📫 Let’s Connect

- 💼 Open to **Internship Opportunities**
- 🌐 GitHub: [github.com/NIKUNJPS](https://github.com/NIKUNJPS)

---
